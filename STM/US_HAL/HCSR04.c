	...
	for(i=0; i<HCSR04_UNITS; i++)
	{
		if((htim->Instance == HCSR04_CfgParam[i].TIM_Instance) && (htim->Channel == gs_HCSR04_info[i].ACTIV_CH))
		{
			...
			else if (gs_HCSR04_info[i].EDGE_STATE == 1)
			{
				...
				gs_HCSR04_info[i].DISTANCE = (gs_HCSR04_info[i].DIFF * 0.01715)/(HCSR04_CfgParam[i].TIM_CLK_MHz/(PS+1));
				gs_HCSR04_info[i].DISTANCE = (int)(gs_HCSR04_info[i].DISTANCE*10)/10.0f;
